...
public class GameControl : MonoBehaviour {
	...
	void Start () {
		...
		StartCoroutine (Bad_Word_Spawn ());
	}

	//called every frame 
	public void FixedUpdate() 
	{
		question.text = solutions [n, 0];
	...
	//QUESTION DOES NOT CHANGE IMMEDIATELY WHEN WORD IS HIT -fixed 
	IEnumerator DisplayQuestion()
	{

		while (n < 100 && TimeLeft > 0) {

			//question.text = solutions [n, 0];
			//print (n + "question"); //used for debugging, remove. 

			yield return new WaitForSeconds (Random.Range(3.0f, 7.0f));
			//spawn position for words 
			Vector3 spawnPosition = new Vector3 (
				Random.Range (-maxWidth / 2, maxWidth / 2),
				transform.position.y,
				-1.0f);
			Quaternion spawnRotation = Quaternion.identity;
			//creates new game object and spawns it 
			GameObject newObject = (GameObject)Instantiate (good_word, spawnPosition, spawnRotation);
			//changes question
			question.text = solutions [n, 0];

			//changes the good word  
			TextMesh hello = newObject.transform.GetComponent<TextMesh> ();
			//changes the answers 
			hello.text = solutions [n, 1];
			hello.fontSize = 8; 
		...
		IEnumerator Bad_Word_Spawn (){

		while (n < 100 && TimeLeft > 0) {
			for (int i = 0; i < 100; i++) {
				yield return new WaitForSeconds (badword_time); 
				Vector3 spawnPosition = new Vector3 (
					                        Random.Range (-maxWidth / 2, maxWidth / 2),
					                        transform.position.y,
					                        -1.0f);
				Quaternion spawnRotation = Quaternion.identity;
				GameObject newObject = (GameObject)Instantiate (bad_word, spawnPosition, spawnRotation);

				TextMesh hello = newObject.transform.GetComponent<TextMesh> ();
				hello.text = solutions [i, Random.Range (1, 2)];
			}
		}
